...
function mavf_slider(
    mavArgs = {
        mavSliderClass          : '.mav-slider',
        mavSliderId             : '',
        mavSlideContainerClass  : ''
    }
) {

    // Query all sliders on the page
    const mavSliders = document.querySelectorAll(mavArgs.mavSliderClass);
    ...
    for ( const mavSlider of mavSliders ){
        ...
        if ( mavTheSliderObj === undefined ) {
            return;
        }

        // Get slider unique number
        const mavUnique = mavTheSliderObj.dataset.unique;

        // Get slider type option
        // Default: 2
        const mavSliderType = mavTheSliderObj.dataset.type ? mavTheSliderObj.dataset.type : 2;

        // Get slider interval option
        // Default: 5000
        const mavInterval = mavTheSliderObj.dataset.interval ? mavTheSliderObj.dataset.interval : 5000;

        // Get Slides container class
        const mavSlideContainerClass = mavArgs.mavSlideContainerClass ? mavArgs.mavSlideContainerClass : `mav-slider-type-${mavSliderType}-ctn`;
